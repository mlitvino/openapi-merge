...
export function loadSpecs(inputs: InputSource[], ctx: MergeContext): void {
  for (const { input } of inputs) {
    ...
    if (isUrl(input)) {
      throwError('load-failed', `URL input requires async loading: ${input}`);
    }
...
async function loadFromUrlAsync(url: string): Promise<unknown> {
  ...
  throwError('load-failed', `Failed to load URL: ${url}. ${message}`);
}
...
function parseSpec(text: string): unknown {
  ...
  throwError('parse-error', 'Failed to parse spec file.');
}
